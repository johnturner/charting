...
module GoogleSpreadsheet
    ...
    class Spreadsheet
        ...
        def duplicate(new_name = "Copy of #@title")
          xml = <<-"EOS"
            <entry xmlns="http://www.w3.org/2005/Atom">
              <id>#{document_feed_url}</id>
              <title>#{new_name}</title>
            </entry>
          EOS
          
          url = "https://docs.google.com/feeds/default/private/full"
          doc = @session.request(:post, url, :data => xml, :auth => :writely)
          ss_url = as_utf8(doc.search(
            "link[@rel='http://schemas.google.com/spreadsheets/2006#worksheetsfeed']")[0]["href"])
        ...
        def add_role email, role
          ...
          url = "https://docs.google.com/feeds/default/private/full/#{self.key}/acl/"
          ...
          EOS
          @session.request(:post, url, :data => xml, :auth => :writely)
        end

        def add_writer email
          add_role email, "writer"
        end
        
        def add_reader email
          add_role email, "reader"
        end
